...
## Bootstrap CI
boot_CI <- function (x, y, B = 5000, seed = 123) {
  set.seed(seed)
  n <- length(x)
  theta <- cor(x, y, method = "pearson")
  theta_star <- mu_star <- rep(0, B)
  
  for (b in 1:B) {
    x_boot <- sample(x, n, replace = TRUE)
    y_boot <- y[sapply(x_boot, function(z) which(x == z))]
    theta_star[b] <- cor(x_boot, y_boot, method = "pearson")
  }
  
  theta_boot_LQ <- quantile(theta_star, 0.025)
  theta_boot_UQ <- quantile(theta_star, 0.975)
  
  CI <- c(2*theta - theta_boot_UQ, 2*theta - theta_boot_LQ)
  return(CI)
}


## rho Fisher test
rho.test <- function (rho, rho0 = 0.95, K, method = "BH") {
  z.center <- log((1 + rho0)/(1 - rho0))/2
  z.sd <- sqrt(K - 3)
  rho.F <- atanh(rho)
  rho.F.center <- (rho.F - z.center) * z.sd
  ps <- p.adjust(pnorm(rho.F.center), method = method)
  return(ps)
}
...
HighOrderScore_singleSample_boot <- function(obj = NULL,
                                             factor = NULL,
                                             reduction = NULL,
                                             sample = NULL,
                                             assay = NULL,
  ...
  ## Find CI of rho using bootstrap
  N_feature <- nrow(res$x_dim)
  boot_CIs <- sapply(1:N_feature, 
                     function(z) boot_CI(x = res$x_dim[z, ], y = res$y_dim[z, ], B = B))
  boot_CIs <- t(boot_CIs)
  colnames(boot_CIs) <- c("rho.lb", "rho.ub")
  rownames(boot_CIs) <- rownames(res$x_dim)
  
  deg_boot <- which(boot_CIs[, 2] < 1)
  n_deg <- length(deg_boot)
  message(paste0(n_deg, " informative latent factors were detected "))
  highorder_diff <- res$diff[, deg_boot]
  
  # squre of z-score matrix
  out <- list()
  out$CIs <- boot_CIs
  out$deg <- rownames(res$x_dim)[deg_boot]
  out$x_dim <- res$x_dim
  out$y_dim <- res$y_dim
...
HighOrderScore_singleSample_test <- function(obj = NULL,
                                             factor = NULL,
                                             reduction = NULL,
                                             sample = NULL,
                                             assay = NULL,
  ...
  ## Calculate pval of testing rho = rho0 vs rho < rho0
  N_feature <- nrow(res$x_dim)
  corr <- sapply(1:N_feature, 
                 function(z) cor(res$x_dim[z, ], res$y_dim[z, ], method = "pearson"))
  pval <- rho.test(rho = corr, rho0 = rho0, K = K, method = method)
  
  deg_test <- which(round(pval, digits = 3) <= 0.05)
  n_deg <- length(deg_test)
  message(paste0(n_deg, " informative latent factors were detected "))
  highorder_diff <- res$diff[, deg_test]
  
  # squre of z-score matrix
  out <- list()
  out$pval <- pval
  out$deg <- rownames(res$x_dim)[deg_test]
  out$x_dim <- res$x_dim
  out$y_dim <- res$y_dim
  ...
  for (k in seq(1, length(sampleLst), 1)) {
    for (l in seq(1, length(sampleLst), 1)) {
      if (l > k) {
        ...
        res <- HighOrderScore_singleSample(
          obj = obj_tp,
          factor = sample,
          sample = sample,
          reduction = NULL,
...
DEGs_plot <- function(dt = NULL, T1 = NULL, T2 = NULL, T3 = NULL, size = 4) {
  ...
  p <- ggplot(dt, aes(x = avg_logFC, y = p_mismatch, color = label)) +
    ...
    geom_text_repel(
      data = dt[dt$label %in% c("DE genes only", "High-order genes only", "Both"), ],
      aes(avg_logFC, p_mismatch, label = gene), nudge_x = 0, size = size
    ) +
    geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = 0.5) +
    scale_colour_manual(values = vega_20) +
    xlim(c(0 - max_lim, max_lim))
...
Diff_Plot <- function(dt = NULL, nullmod = NULL) {
  ...
  dt <- data.frame(
    "X2" = c(dt, nullmod[seq(1, length(dt), 1)]),
  ...
  geom_text_repel(
    data = dt[!dt$label == "filter", ], max.overlaps = Inf,
